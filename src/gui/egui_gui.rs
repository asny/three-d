...
impl GUI {
    ...
    fn paint_mesh(
        ...
        let position_buffer = Buffer::new_with_data(&self.context, &positions)?;
        let uv_buffer = Buffer::new_with_data(&self.context, &uvs)?;
        let color_buffer = Buffer::new_with_data(&self.context, &colors)?;
