...
impl Shader
{
    ...
    {
        let shader_type = if kind == consts::VERTEX_SHADER {"Vertex shader".to_string()} else {"Fragment shader".to_string()};
    ...
    pub fn attach_shader(&self, program: &crate::gl::Program)
    {
        self.gl.attach_shader(program, &self.id);
    }

    pub fn detach_shader(&self, program: &crate::gl::Program)
    {
