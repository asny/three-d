...
impl FogEffect {
    ...
    pub fn apply(&self, viewport: Viewport, camera: &Camera, depth_texture: &dyn Texture, time: f32) -> Result<(), Error>
    {
        ...
        self.image_effect.use_uniform_mat4("viewProjectionInverse", &(camera.projection() * camera.view()).invert().unwrap())?;
        self.image_effect.use_uniform_vec3("fogColor", &self.color)?;
        self.image_effect.use_uniform_float("fogDensity", &self.density)?;
        self.image_effect.use_uniform_float("animation", &self.animation)?;
        self.image_effect.use_uniform_float("time", &(0.001 * time))?;
        self.image_effect.use_uniform_vec3("eyePosition", camera.position())?;
