            ...
            &format!("
                ...
                ", &dir_uniform, &spot_uniform, &point_uniform,
                     if directional_lights > 0 || spot_lights > 0 || point_lights > 0 {
                         "Surface surface = get_surface(); vec4 surfaceColor = surface.color;"} else {"vec4 surfaceColor = get_surface_color();"},
...
use crate::core::*;
use crate::light::*;
fn bind_lights(effect: &Program, ambient_light: Option<&AmbientLight>, directional_lights: &[&DirectionalLight],
               spot_lights: &[&SpotLight], point_lights: &[&PointLight]) -> Result<(), Error>
{
    // Ambient light
    effect.use_uniform_vec3("ambientColor",
                            &ambient_light.map(|light| light.color * light.intensity)
