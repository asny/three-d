...
impl Camera {
    ...
    pub fn pick(&self, position: Vec3, target: Vec3, objects: &[&dyn Pickable]) -> Result<Vec3, Error>  {
        ...
        let camera = Camera::new_orthographic(&self.context, position, target, up, 0.01, 0.01, max_depth)?;
