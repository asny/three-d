...
pub fn main() {
    ...
    let mut window1 = create_window(&event_loop);
    let mut window2 = create_window(&event_loop);

    event_loop.run(move |event, target, control_flow| {
        window1(&event, target, control_flow);
        window2(&event, target, control_flow);
    })
}


fn create_window(event_loop: &EventLoopWindowTarget<()>) -> impl FnMut(&winit::event::Event<'_, ()>, &EventLoopWindowTarget<()>, &mut ControlFlow) {
    ...
    window.get_render_loop(move |frame_input| // Begin a new frame with an updated frame input
    {
