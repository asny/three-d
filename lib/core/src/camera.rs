...
impl Camera
{
    ...
    {
        let mut camera = Camera {matrix_buffer: UniformBuffer::new(gl, &vec![16, 16]).unwrap(), position, target, up, view: Mat4::identity(), projection: Mat4::identity(), screen2ray: Mat4::identity()};
