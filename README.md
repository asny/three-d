<div align="center">
  <img width="50%" src="https://asny.github.io/three-d/0.20/logo.png" alt="three-d logo">
</div>
...
| [`window`](https://docs.rs/three-d/0/three_d/window/)     | Window functionality on cross-platform native and web (requires the `window` feature to be enabled which is the default). Using this module makes it easy to get started, still, in some cases, it is desirable to replace the default `Window` implementation with a custom [winit](https://github.com/rust-windowing/winit) window as exemplified in the `winit_window` example. This module can also be replaced entirely by anything that provides an OpenGL or WebGL2 graphics context, for example [winit](https://github.com/rust-windowing/winit) and [glutin](https://github.com/rust-windowing/glutin), [wasm-bindgen](https://rustwasm.github.io/wasm-bindgen/introduction.html) or [eframe](https://github.com/emilk/egui/tree/master/crates/eframe). |
